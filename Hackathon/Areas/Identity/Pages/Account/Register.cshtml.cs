...
namespace Hackathon.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await _userManager.AddToRoleAsync(user, "User"); /*Міняти для створення Адміна*/
    ...
    $"<p>Вітаємо Вас з найкращими новинами!</p>Для успішного завершення реєстрації та активації вашого облікового запису, перейдіть за <a href='{HtmlEncoder.Default.Encode(callbackUrl)}' style='color: #4CAF50; text-decoration: none; font-weight: bold;'>цим посиланням</a>.</p>Щиро вдячні за обрання нашого сервісу! Нам важливо, щоб кожен момент з нами був особливим.</p>З найкращими побажаннями, Команда <b>SSG-25K</b>");
        ...
        private HackathonUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<HackathonUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(HackathonUser)}'. " +
                    $"Ensure that '{nameof(HackathonUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<HackathonUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<HackathonUser>)_userStore;
        }
    }
}
